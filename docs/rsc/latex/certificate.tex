...
% First file number is inline with the fullname. all numbers float right.
$for(respondents)$
  \noindent
  \textbf{$it.full_name$} \hfill \textbf{File No.: $it.file_number_one$}
  $for(it.file_numbers_rest)$
    \\
    \makebox[\linewidth][r]{\textbf{$it$}}
  $endfor$
  \\
$endfor$
